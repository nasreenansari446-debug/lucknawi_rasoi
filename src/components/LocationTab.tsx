      ...
      <section className="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 items-start">
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="bg-brand-surface-low border border-brand-outline-variant/15 rounded-2xl p-4 md:p-6 shadow-xl relative overflow-hidden royal-glow">
            ...
            <div className="aspect-video w-full rounded-xl bg-[#090909] border border-brand-outline-variant/10 p-3 flex items-center justify-center relative overflow-hidden">
              <svg 
                viewBox="0 0 800 450" 
                className="w-full h-full select-none"
                style={{ filter: "drop-shadow(0 4.5px 12px rgba(0,0,0,0.5))" }}
              >
                {/* Grid Gridlines overlay */}
                <g stroke="#ffffff" strokeWidth="0.5" strokeOpacity="0.03" strokeDasharray="3,3">
                  {[...Array(9)].map((_, i) => (
                    <line key={`x-${i}`} x1={(i + 1) * 80} y1="0" x2={(i + 1) * 80} y2="450" />
                  ))}
                  {[...Array(5)].map((_, i) => (
                    <line key={`y-${i}`} x1="0" y1={(i + 1) * 75} x2="800" y2={(i + 1) * 75} />
                  ))}
                </g>

                {/* Saryu River Canal Branch */}
                <path 
                  d="M0,50 Q200,90 400,60 T800,80" 
                  fill="none" 
                  stroke="#26415e" 
                  strokeWidth="20" 
                  strokeLinecap="round"
                  opacity="0.3"
                />
                <text x="380" y="40" fill="#2d4a6b" fontSize="10" fontWeight="bold" letterSpacing="2" className="font-sans italic">SARAYU CANAL BYPASS</text>

                {/* Roads Scheme */}
                {/* Lucknow Highway Avenue */}
                <line x1="100" y1="450" x2="600" y2="0" stroke="#1d1d1d" strokeWidth="12" strokeLinecap="round" />
                {/* Lucknow Road (Main heading is Lucknow road bahraich) */}
                <line x1="0" y1="280" x2="800" y2="180" stroke="#1d1d1d" strokeWidth="10" strokeLinecap="round" />
                <line x1="250" y1="0" x2="250" y2="450" stroke="#1d1d1d" strokeWidth="8" strokeLinecap="round" />

                {/* Road Names annotation */}
                <text x="180" y="420" fill="gray" fontSize="9" fontWeight="bold" letterSpacing="1" transform="rotate(-42, 180, 420)">LUCKNOW BYPASS</text>
                <text x="500" y="210" fill="gray" fontSize="9" fontWeight="bold" letterSpacing="1" transform="rotate(-7, 500, 210)">LUCKNOW ROAD</text>
                
                {/* Landmark 1: Diggaj Chauraha */}
                ...
                <g transform="translate(420, 150)">
                  <circle cx="0" cy="0" r="12" fill="#121212" stroke="#C5A059" strokeWidth="1" strokeOpacity="0.3" />
                  <circle cx="0" cy="0" r="4" fill="#C5A059" fillOpacity="0.6" />
                  <text x="14" y="4" fill="darkgray" fontSize="9" fontWeight="bold" className="font-sans">Hotel Vaishnavi</text>
                </g>

                {/* Landmark 3: Indira Stadium */}
                ...
                <g transform="translate(480, 185)">
                  {/* Glowing radar rings */}
                  <circle cx="0" cy="0" r="35" fill="#C5A059" fillOpacity="0.03" className="animate-pulse" />
                  <circle cx="0" cy="0" r="22" fill="none" stroke="#C5A059" strokeWidth="0.5" strokeDasharray="3,2" />
                  <circle cx="0" cy="0" r="12" fill="#C5A059" fillOpacity="0.15" />
                  
                  {/* Pin Point */}
                  <path d="M0,-12 C-6,-12 -6,-4 0,0 C6,-4 6,-12 0,-12 Z" fill="#C5A059" />
                  <circle cx="0" cy="-7" r="2.5" fill="#080808" />
                  
                  {/* Primary text label */}
                  <rect x="-65" y="-36" width="130" height="18" rx="4" fill="#080808" stroke="#C5A059" strokeWidth="1" />
                  <text x="0" y="-24" fill="#C5A059" fontSize="9" fontWeight="bold" textAnchor="middle" letterSpacing="1" className="font-display">LUCKNOWI RASOI</text>
                </g>
